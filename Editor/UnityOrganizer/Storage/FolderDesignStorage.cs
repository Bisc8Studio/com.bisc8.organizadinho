...
namespace UnityOrganizer.Editor.Storage
{
    [Serializable]
    public class FolderDesignEntry
    {
        ...
        public UnityOrganizerColorMode colorMode = UnityOrganizerColorMode.Pastel;
        public float hue = ColorPaletteUtility.DefaultHue;
        public Color customColor = ColorPaletteUtility.GetBaseColor(UnityOrganizerColorMode.Pastel, ColorPaletteUtility.DefaultHue);
    ...
    {
        private const string ProjectSettingsAssetPath = "ProjectSettings/UnityOrganizer/FolderDesignStorage.asset";
        ...
        public FolderDesignEntry GetOrCreateEntry(string folderGuid)
        {
            ...
            {
                guid = folderGuid,
                colorMode = UnityOrganizerColorMode.Pastel,
                hue = ColorPaletteUtility.DefaultHue,
                customColor = ColorPaletteUtility.GetBaseColor(UnityOrganizerColorMode.Pastel, ColorPaletteUtility.DefaultHue)
            };
            entries.Add(entry);
            return entry;
        }
        ...
        private void NormalizeEntryColors()
        {
            for (var index = 0; index < entries.Count; index++)
            {
                ...
                if (!Enum.IsDefined(typeof(UnityOrganizerColorMode), entry.colorMode))
                    entry.colorMode = UnityOrganizerColorMode.Pastel;

                entry.hue = ColorPaletteUtility.NormalizeHue(entry.hue);
                if (entry.customColor.a <= 0f)
                    entry.customColor = ColorPaletteUtility.GetBaseColor(UnityOrganizerColorMode.Pastel, ColorPaletteUtility.DefaultHue);
