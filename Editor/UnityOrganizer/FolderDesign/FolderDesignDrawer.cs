...
using UnityOrganizer.Editor.Storage;
using UnityOrganizer.Editor.UI;
using UnityOrganizer.Editor.Utilities;

namespace UnityOrganizer.Editor.FolderDesign
{
...
public static class FolderDesignDrawer
{
    ...
    private static void OnProjectGUI(string guid, Rect selectionRect)
    {
        ...
        if (UnityOrganizerUserPreferences.MatchesPopupShortcut(Event.current) &&
        ...
        if (!renderData.Style.HasVisualOverride ||
            (!UnityOrganizerUserPreferences.EnableFolderColors && !renderData.Style.HasCustomIcon))
            return;

        var isIconView = selectionRect.height > 20f;
        var iconRect = GetFolderIconRect(selectionRect, isIconView);

        if (UnityOrganizerUserPreferences.EnableFolderColors)
        {
